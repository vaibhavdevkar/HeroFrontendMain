...
const OxsilanConductivity = () => {
  ...
  const fetchData = async (interval) => {
    ...
    try {
      const response = await axios.get(`http://localhost:5002/api/pumpmotorcurrent/OXConductivity?startDate=${startDate}&endDate=${endDate}`);
      const data = response.data.map(record => ({
        x: new Date(record.Date_Time).getTime(),
        y: record.OXConductivity
      }));
        ...
        <div className="row">
          <div className="col-md-12 mb-4">
            <div className="card">
              ...
              <h4 className="font-weight-bold">Conductivity Chart</h4>
...
export default OxsilanConductivity