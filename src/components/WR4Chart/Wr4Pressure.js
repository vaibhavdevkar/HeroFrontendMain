...
const Wr4Pressure = () => {
  ...
  const fetchData = async (interval) => {
    ...
    try {
      const response = await axios.get(`http://localhost:5002/api/pumpmotorcurrent/WR4CIRPRSRLBL?startDate=${startDate}&endDate=${endDate}`);
      const data = response.data.map(record => ({
        x: new Date(record.Date_Time).getTime(),
        y: record.WR4CIRPRSRLBL
      }));
