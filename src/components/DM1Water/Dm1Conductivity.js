...
const Dm1Conductivity = () => {
  ...
  const fetchData = async (interval) => {
    ...
    try {
      const response = await axios.get(`http://localhost:5002/api/pumpmotorcurrent/DMConductivity?startDate=${startDate}&endDate=${endDate}`);
      const data = response.data.map(record => ({
        x: new Date(record.Date_Time).getTime(),
        y: record.DMConductivity
      }));
...
export default Dm1Conductivity