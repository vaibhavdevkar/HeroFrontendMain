...
const DegreasePressure = () => {
  const [chartData, setChartData] = useState({
    series: [{
      name: 'Pump Motor Circulation Pressure',
      data: []
    }],
    options: {
      chart: {
  ...
  const fetchData = async (interval) => {
    ...
    try {
      const response = await axios.get(`http://localhost:5002/api/pumpmotorcurrent/DCIRPRSRLBL?startDate=${startDate}&endDate=${endDate}`);
      const data = response.data.map(record => ({
        x: new Date(record.Date_Time).getTime(),
        y: record.DCIRPRSRLBL
      }));
        ...
        <div className="row">
          <div className="col-md-12 mb-4">
            <div className="card">
              ...
              <h4 className="font-weight-bold">Circulation Pressure</h4>
...
export default DegreasePressure