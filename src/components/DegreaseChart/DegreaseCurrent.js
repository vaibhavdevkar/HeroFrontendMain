...
const DegreaseCurrent = () => {
  ...
  const fetchData = async (interval) => {
    ...
    try {
      const response = await axios.get(`http://localhost:5002/api/pumpmotorcurrent/DPUMPMOTLBL?startDate=${startDate}&endDate=${endDate}`);
      const data = response.data.map(record => ({
        x: new Date(record.Date_Time).getTime(),
        y: record.DPUMPMOTLBL
      }));
        ...
        <div className="row">
          <div className="col-md-12 mb-2">
            <div className="card">
              ...
              <h4 className="font-weight-bold">Current Chart</h4>
...
export default DegreaseCurrent