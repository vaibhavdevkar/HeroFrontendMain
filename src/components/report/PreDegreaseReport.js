...
const PreDegreaseReport = () => {
  const [headline, setHeadline] = useState("Pre Degrease Report");
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('http://localhost:5002/api/predegreaserecord');
  ...
  const filterData = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(`http://localhost:5002/api/predegreaserecord?startDate=${startDate}&endDate=${endDate}`);
      const jsonData = await response.json();
      setData(jsonData);
      setIsLoading(false);
    } catch (error) {
  ...
  const exportToExcel = () => {
    ...
    const formattedData = data.map(row => ({
      ...
      PDPUMPSTATLBL: row.PDPUMPSTATLBL,
      PDPUMPMOTLBL: row.PDPUMPMOTLBL,
      PDTANKLVLLBL: row.PDTANKLVLLBL,
      PDCIRPRSRLBL: row.PDCIRPRSRLBL,
      PDTEMPLBL: row.PDTEMPLBL,
      // PDSOFTWATERLBL: row.PDSOFTWATERLBL
    }));
            ...
            <tbody>
              {data.map((row, index) => (
                <tr key={index}>
                  ...
                  <td>{row.PDPUMPSTATLBL}</td>
                  <td>{row.PDPUMPMOTLBL}</td>
                  <td>{row.PDTANKLVLLBL}</td>
                  <td>{row.PDCIRPRSRLBL}</td>
                  <td>{row.PDTEMPLBL}</td>
                  {/* <td>{row.PDSOFTWATERLBL}</td> */}
                </tr>
              ))}
            </tbody>
          </Table>
...
export default PreDegreaseReport