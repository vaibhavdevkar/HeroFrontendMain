...
const FreshWaterReport = () => {
  const [headline, setHeadline] = useState("Fresh DM Water Report");
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('http://localhost:5002/api/freshwaterreport');
  ...
  const filterData = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(`http://localhost:5002/api/freshwaterreport?startDate=${startDate}&endDate=${endDate}`);
      const jsonData = await response.json();
      setData(jsonData);
      setIsLoading(false);
    } catch (error) {
  ...
  const exportToExcel = () => {
    ...
    const formattedData = data.map(row => ({
      ...
      FDWPUMPSTATLBL: row.FDWPUMPSTATLBL,
      FDWPUMPMOTLBL: row.FDWPUMPMOTLBL,
      FDWTANKLVLLBL: row.FDWTANKLVLLBL,
      FDWCIRPRSRLBL: row.FDWCIRPRSRLBL,
      FDWPHLBL: row.FDWPHLBL,
      FDWConductivity: row.FDWConductivity
    }));
            ...
            <tbody>
              {data.map((row, index) => (
                <tr key={index}>
                  ...
                  <td>{row.FDWPUMPSTATLBL}</td>
                  <td>{row.FDWPUMPMOTLBL}</td>
                  <td>{row.FDWTANKLVLLBL}</td>
                  <td>{row.FDWCIRPRSRLBL}</td>
                  <td>{row.FDWPHLBL}</td>
                  <td>{row.FDWConductivity}</td>
                </tr>
              ))}
            </tbody>
          </Table>
