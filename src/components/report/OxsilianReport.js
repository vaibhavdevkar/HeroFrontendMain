...
const OxsilianReport = () => {
  const [headline, setHeadline] = useState("Oxsilan Report");
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('http://localhost:5002/api/oxsilanreport');
  ...
  const filterData = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(`http://localhost:5002/api/oxsilanreport?startDate=${startDate}&endDate=${endDate}`);
      const jsonData = await response.json();
      setData(jsonData);
      setIsLoading(false);
    } catch (error) {
  ...
  const exportToExcel = () => {
    ...
    const formattedData = data.map(row => ({
      ...
      OXPUMPSTATLBL: row.OXPUMPSTATLBL,
      OXPUMPMOTLBL: row.OXPUMPMOTLBL,
      OXTANKLVLLBL: row.OXTANKLVLLBL,
      OXCIRPRSRLBL: row.OXCIRPRSRLBL,
      OXPHLBL: row.OXPHLBL,
      OXConductivity: row.OXConductivity
    }));
            ...
            <tbody>
              {data.map((row, index) => (
                <tr key={index}>
                  ...
                  <td>{row.OXPUMPSTATLBL}</td>
                  <td>{row.OXPUMPMOTLBL}</td>
                  <td>{row.OXTANKLVLLBL}</td>
                  <td>{row.OXCIRPRSRLBL}</td>
                  <td>{row.OXPHLBL}</td>
                  <td>{row.OXConductivity}</td>
                </tr>
              ))}
            </tbody>
          </Table>
