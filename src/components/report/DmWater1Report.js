...
const DmWater1Report = () => {
  const [headline, setHeadline] = useState("DM Water 1 Report");
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('http://localhost:5002/api/dmwater1report');
  ...
  const filterData = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(`http://localhost:5002/api/dmwater1report?startDate=${startDate}&endDate=${endDate}`);
      const jsonData = await response.json();
      setData(jsonData);
      setIsLoading(false);
    } catch (error) {
  ...
  const exportToExcel = () => {
    ...
    const formattedData = data.map(row => ({
      ...
      DMPUMPSTATLBL: row.DMPUMPSTATLBL,
      DMPUMPMOTLBL: row.DMPUMPMOTLBL,
      DMTANKLVLLBL: row.DMTANKLVLLBL,
      DMCIRPRSRLBL: row.DMCIRPRSRLBL,
      DMPHLBL: row.DMPHLBL,
      DMConductivity: row.DMConductivity
      // DMTEMPLBL: row.DMTEMPLBL,
      // DMSOFTWATERLBL: row.DMSOFTWATERLBL
    }));
            ...
            <tbody>
              {data.map((row, index) => (
                <tr key={index}>
                  ...
                  <td>{row.DMPUMPSTATLBL}</td>
                  <td>{row.DMPUMPMOTLBL}</td>
                  <td>{row.DMTANKLVLLBL}</td>
                  <td>{row.DMCIRPRSRLBL}</td>
                  <td>{row.DMPHLBL}</td>
                  <td>{row.DMConductivity}</td>
                </tr>
              ))}
            </tbody>
          </Table>
