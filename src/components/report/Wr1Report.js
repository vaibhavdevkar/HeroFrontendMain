...
const Wr1Report = () => {
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('http://localhost:5002/api/wr1report');
  ...
  const filterData = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(`http://localhost:5002/api/wr1report?startDate=${startDate}&endDate=${endDate}`);
      const jsonData = await response.json();
      setData(jsonData);
      setIsLoading(false);
    } catch (error) {
  ...
  const exportToExcel = () => {
    ...
    const formattedData = data.map(row => ({
      ...
      WR1PUMPSTATLBL: row.WR1PUMPSTATLBL,
      WR1PUMPMOTLBL: row.WR1PUMPMOTLBL,
      WR1TANKLVLLBL: row.WR1TANKLVLLBL,
      WR1CIRPRSRLBL: row.WR1CIRPRSRLBL,
      WR1TEMPLBL: row.WR1TEMPLBL,
      // WR1SOFTWATERLBL: row.WR1SOFTWATERLBL
    }));
            ...
            <tbody>
              {data.map((row, index) => (
                <tr key={index}>
                  ...
                  <td>{row.WR1PUMPSTATLBL}</td>
                  <td>{row.WR1PUMPMOTLBL}</td>
                  <td>{row.WR1TANKLVLLBL}</td>
                  <td>{row.WR1CIRPRSRLBL}</td>
                  <td>{row.WR1TEMPLBL}</td>
                  {/* <td>{row.WR1SOFTWATERLBL}</td> */}
                </tr>
              ))}
            </tbody>
          </Table>
